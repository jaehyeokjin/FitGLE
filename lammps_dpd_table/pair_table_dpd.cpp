...
void PairTable::compute(int eflag, int vflag)
{
  ...
  double randnum_para, randnum_vertical1, randum_vertical2;
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (tabstyle == LOOKUP) {
          ...
          fpair = factor_lj * tb->f[itable] - friction + randnum * sqrt(2*k*T*parallelGamma); //Double Check This!
          //vertical vector 1
          double rr = sqrt(rsq);
          randnum_para = random->gaussian();
          ...
          double randomv1 = randnum_vertical1 * sqrt(2*kT*verticalGamma);  //Double Check

          randnum_para = random->gaussian();
	  randnum_vertical2 = random->gaussian();
          double r2vx = (dely * r1vz - delz * r1vy) / rr;
          double r2vy = -delx * r1vz / rr;
          double r2vz = delx * r1vy / rr;
          double randomv2 = randnum_vertical2 * sqrt(2*kT*verticalGamma);  //Double Check
          ...
          double friction = parallelGamma * v_para;
          randnum_para = random->gaussian();
          fpair = factor_lj * value - friction + randnum * sqrt(2*k*T*parallelGamma); //Double Check This!
          //vertical vector 1
          double rr = sqrt(rsq);
          randnum_para = random->gaussian();
          randnum_vertical1 = random->gaussian();
...
void PairTable::settings(int narg, char **arg)
{
  ...
  // new settings
  kT = force->boltz * atoi(arg[0]);

  if (strcmp(arg[1],"lookup") == 0) tabstyle = LOOKUP;
  else if (strcmp(arg[1],"linear") == 0) tabstyle = LINEAR;
  else if (strcmp(arg[1],"spline") == 0) tabstyle = SPLINE;
  else if (strcmp(arg[1],"bitmap") == 0) tabstyle = BITMAP;
  else error->all(FLERR,"Unknown table style in pair_style command");

  tablength = force->inumeric(FLERR,arg[2]);
...
void PairTable::compute_table(Table *tb)
{
  ...
  if (tabstyle == SPLINE) {
    ...
    ppn = (tb->p[tlm1] - splint(tb->rfile,tb->pfile,tb->p2file,tb->ninput,sqrt(rsq2))) / (secant_factor * tb->delta);
    ...
    rsq2 = tb->cut * tb->cut;
    rsq1 = rsq2 - secant_factor * tb->delta;
    ppn = (tb->v[tlm1] - splint(tb->rfile,tb->vfile,tb->v2file,tb->ninput,sqrt(rsq2))) / (secant_factor * tb->delta);
