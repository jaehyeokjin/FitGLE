...
inline std::vector<double> FitGLE::paraVerticalVelocity(int i, int j)
{
    double dx = trajFrame->positions[i][0] - trajFrame->positions[j][0];
    if (dx > 0.5 * info->boxLength) dx = dx - info->boxLength;
    if (dx < -0.5 * info->boxLength) dx = dx + info->boxLength;
    ...
    for (int i = 0; i < nall; i++)
    {
        for (int j = i + 1; j < nall; j++)
        {
            ...
            if (rij < info->end)
            {
                ...
                //printf("rij = %lf, %d %d\n", rij, i, j);    
                std::vector<double> dv = paraVerticalVelocity(i, j);
                ...
                for (int m=0; m<nSplines; m++)
                {
		         ...
		         continue;
                     ...
                     frameMatrix[m + nSplines][3*j] -= phim * dv[3];
                     frameMatrix[m + nSplines][3*j + 1] -= phim * dv[4];
                     frameMatrix[m + nSplines][3*j + 2] -= phim * dv[5];
                }
            }  
        }
    }
